...
function run_external_backend_sweep(
    ...
    # External engines have no surrogate mode (their APIs are numerical-only),
    # so a sweep re-propagates the full circuit at every angle. LOWESA keeps
    # its RX-only sweep rule; any other family sweeps every rotation angle.
    ...
    for angle_index in selected
        ...
        absolute_error = has_reference ? abs(result.expectation - reference) : nothing
        if has_reference
    ...
    if is_lowesa
        ...
        metadata["max_freq"] = Int(get(spec.truncation, "max_freq", 40))
        metadata["max_weight"] = Int(get(spec.truncation, "max_weight", 8))
